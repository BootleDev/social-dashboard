...
4. **Per-platform settle windows:** `FB_SETTLE_DAYS = 3` (90% within 72h; up from today's 2), `IG_SETTLE_DAYS = 21`. Named constants, tunable.
5. **Hard constraint on the IG window (why 21, not 30):** posts and account insights are fetched for the **last 30 days** only, so a row is re-written each run *only while it's inside that window*. A row flips to `settled` the first run where `age > threshold` (day `threshold+1`). If the threshold **≥** the ~30d window, that flip lands the day *after* the row drops out of the window — so it's never re-written and stays `pending` forever (a silent bug). The threshold must sit under the window with margin: runs left to write `settled` after the flip = `30 − threshold`. **21** leaves ~9 such runs — robust against a run of failed daily executions and the fuzzy 30-day boundary (a post published "30 days ago" straddles the cutoff by time-of-day) — while capturing the bulk of IG Reel accrual (front-loaded in ~2-3 weeks; the 3-6 week tail is a slow drip). Values ≥ ~28 are fragile (1-2 runs of margin); 30 is the bug. Chosen over 14 for completeness (Alex, 2026-07-02) since completeness was prioritised over freshness. The full "2-6 weeks" tail would require widening the fetch window (rejected — enlarges every run's payload; prior payload-bloat incident MARKETING-35).
...
const IG_SETTLE_DAYS = 21;  // IG Reels front-load accrual in ~2-3 weeks; MUST stay under the ~30d fetch/re-emit window (see decision 5)
...
**B2. `MethodologyContent.tsx` — honest copy.** Update the settle explainer (line ~90 "settle over ~1-2 days"; line ~578 same) to describe per-platform windows (FB ~3d, IG ~21d) and that the ER comparison shows settled data only.
...
- **Re-emit healing:** a post row flips `pending → settled` automatically once it ages past its window, because it is re-emitted every run through the 30d fetch window (`IG_SETTLE_DAYS=21 < 30`, ~9 runs of margin after the flip). Same mechanism already covers the account rows' ~30d re-emit.
